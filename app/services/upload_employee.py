...
from app.enums import MatchyComparer,FieldType,ConditionProperty,ContractType,Gender,Role
...
fields_check = {
    ...
    "gender": (
        ...
        f"Possible values are : {Gender.get_possible_values()}",
    ),
    "contract_type": (
...
def validate_employees_data_and_upload(employees: list,force_upload: bool,background_tasks: BackgroundTasks,db: Session):
    try:
        ...
        for emp in employees_to_add:
            new_token = models.AccountActivation(employee_id=emp.id, email=emp.email, token=uuid.uuid4())
...
def upload(entry: schemas.UploadEntry,background_tasks: BackgroundTasks,db: Session,):
